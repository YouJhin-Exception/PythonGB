...
botSymbol = emoji.emojize(':hollow_red_circle:')
playerSymbol = emoji.emojize(':cross_mark:')
...
async def callbackInline(call):
    ...
    if (call.message):
       ...
       if losebool:
           clear()
           await bot.send_message(call.message.chat.id, emoji.emojize(' Я победил!!! :winking_face_with_tongue::victory_hand:'))
           losebool = False
           gameIsStart = False
